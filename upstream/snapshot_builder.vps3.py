...
# Cuentas REALES consolidadas en instalaciones portables (2026-07-27). El glob de
# Program Files no las ve, y mt5.initialize(path=) ARRANCA el terminal que toca:
# dejar aqui las carpetas viejas hacia que el builder reabriera 25425 y 32081 cada
# ciclo, duplicandolas. Las viejas se renombraron a .OLD y se anade el escritorio.
MT5_GLOB_REALES = r"C:\Users\Administrator\Desktop\CUENTAS-REALES\*\terminal64.exe"
WINDOW_DAYS = 365
...
def _terminales_corriendo():
    """Rutas de terminal64.exe EN EJECUCION (minusculas).

    pywin32 / EnumProcesses, SIN shell. La version anterior llamaba a powershell por
    subprocess y su comando quedo corrupto (`{ \\.Path }` en vez de `{ $_.Path }`), asi
    que fallaba SIEMPRE y devolvia un set vacio: las 5 cuentas REALES desaparecieron del
    dashboard entero (incidente 2026-07-27, snapshot con bots=0). Sin shell no hay
    comillas que se puedan corromper ni PATH que dependa del contexto de arranque.
    """
    rutas = set()
    try:
        import win32api
        ...
        print(f"WARN: pywin32 no disponible ({exc}) -- no se leeran cuentas reales")
        return rutas
...
def discover_terminals():
    demos = set(glob.glob(MT5_GLOB))
    # CUENTAS REALES: nunca se abren desde aqui. mt5.initialize(path=) ARRANCA el
    # terminal si esta cerrado, y el owner exige que sus cuentas reales las abra
    # solo el (regla 2026-07-27). Por eso solo entran las que YA estan corriendo:
    # se leen, no se abren.
    corriendo = _terminales_corriendo()
    todas_reales = glob.glob(MT5_GLOB_REALES)
    reales = {p for p in todas_reales if p.lower() in corriendo}
    # El fallo anterior fue SILENCIOSO: dashboard vacio sin una sola linea de aviso.
    # Con esto, un 0/5 con terminales abiertos se ve en snapshot.log al instante.
    print(f"reales_vistas={len(reales)}/{len(todas_reales)}")
    return sorted(demos | reales)
